...
Andrej Karpathy의 관찰을 바탕으로 한 하네스다. 에이전트의 경계·검증·스코프를 고정해 침묵 가정, 과설계, 스코프 확장, 모호한 “됐다” 선언을 줄인다.

적용 범위: 이 저장소에서 Cursor가 코드·설정·명령을 제안하거나 실행할 때(채팅, Composer, 터미널 에이전트 등). 저장소 루트의 .cursorrules에 의해 본 파일(AGENTS.md) 필독이 강제된다. 에이전트는 작업을 시작하기 전에 본 문서를 읽고, 상세 문구·예시는 CLAUDE.md를 함께 따른다.
...
.cursorrules	IDE 규칙; 본 파일과 CLAUDE.md 필독을 명시한다.
