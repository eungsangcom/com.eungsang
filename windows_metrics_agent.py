...
def _probe_local_service(label: str, port: int, path: str) -> dict:
    ...
    try:
        ...
        try:
            ...
            reachable = False
    return {"label": label, "port": port, "reachable": reachable, "latencyMs": latency_ms}
...
def _services() -> list[dict]:
    ...
    with ThreadPoolExecutor(max_workers=len(SERVICE_PORTS) or 1) as pool:
        futures = [
            pool.submit(
                _probe_local_service,
                label,
                port,
                health_paths.get(label, "/health"),
            )
